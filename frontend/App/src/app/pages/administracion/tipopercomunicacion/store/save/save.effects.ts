...
export class SaveEffects {
    ...
    this.actions.pipe(
      ...
      switchMap((request: TipoPerComunicacionCreateRequest) =>
        this.httpClient.post<TipoPerComunicacionResponse[]>(`${environment.url}api/TipoPersonaComunicacion`, request)
          .pipe(
            delay(1000),
            map((tipopercomunicaciones: TipoPerComunicacionResponse[]) => new fromActions.CreateSuccess(tipopercomunicaciones)),
            catchError(err => {
  ...
  delete: Observable<Action> = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap((request: TipoPerComunicacionCreateRequest) =>
        this.httpClient.post<TipoPerComunicacionResponse[]>(`${environment.url}api/TipoPersonaComunicacion/DeleteTipoPerComunicacion`, request)
          .pipe(
            delay(1000),
            map((tipopercomunicaciones: TipoPerComunicacionResponse[]) => new fromActions.CreateSuccess(tipopercomunicaciones)),
            catchError(err => {
