...
import { ClienteUsuarioResponse as Response,  ClienteUsuarioRequest as Request, ClienteUsuarioRequestUpdate as RequestUpdate } from '../../store/save';
...
@Component({
  selector: 'app-clienteusuario-datosgenerales',
  templateUrl: './clienteusuario-datosgenerales.component.html',
  styleUrls: ['./clienteusuario-datosgenerales.component.scss']
})
export class ClienteusuarioDatosgeneralesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._routeParams.paramMap.subscribe(params => {
      if(params.get('id')) {
        ...
        this.store.dispatch(new fromList.GetClienteUsuariobyid(this.ID));

        this.loading$ = this.store.pipe(select(fromList.getLoading))!;

        this.store.pipe(select(fromList.getClienteUsuariobyid))
          .subscribe(data => {
            if(data){
  ...
  inputBlur(): void {
    ...
    ruti = rutiformat.substring(0, rutiformat.length -1)
      ...
      this.store.dispatch(new fromList.GetClienteUsuariobyrut(ruti));
      this.loading$ = this.store.pipe(select(fromList.getLoading))!;


      this.store.pipe(select(fromList.getClienteUsuariobyrut))
        .subscribe(data => {
          if(data){
  ...
  onguardardatosgenerales(): void {
    ...
    if(this.Form.valid){
      ...
      if(this.ID){
        ...
        this.store.dispatch(new fromList.UpdateClienteUsuario(RequestUpdateData));
        ...
        this.store.dispatch(new fromList.CreateClienteUsuario(Request));
