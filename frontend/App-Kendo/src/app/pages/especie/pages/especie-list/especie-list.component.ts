...
export class EspecieListComponent implements OnInit {
  ...
  public loadData(): Observable<Response[] | null> {
    ...
    const datafull =  this.store.pipe(select(fromList.getEspecies));

    if(this.filtro ){    
      const datafullfilter = datafull.pipe(
        map((data : Response[] | null) => data!.filter(valor => valor.esp03nombre.toLowerCase().includes(this.filtro.toLowerCase())))
        )
  ...
  ngOnInit(): void {
    this.store.dispatch(new fromList.ReadEspecieDatosgenerales());
    this.loading$ = this.store.pipe(select(fromList.getLoading))!;
    this.loadData();
  }

  OnNuevo(){
    this._Route.navigate(['/dashboard/especies/especies/datosgenerales']);
  }

  OnEditar(id: number){
    this._Route.navigate(['/dashboard/especies/especies/datosgenerales/', id.toString()]);
  }
