...
export class TemporadaListComponent implements OnInit {
  ...
  public loadData(): Observable<Response[] | null> {
    ...
    const datafull =  this.store.pipe(select(fromList.getTemporadas));

    if(this.filtro ){    
      const datafullfilter = datafull.pipe(
        map((data : Response[] | null) => data!.filter(valor => valor.temp01nombre!.toLowerCase().includes(this.filtro.toLowerCase())))
        )
  ...
  OnNuevo(){
    this._Route.navigate(['/dashboard/temporada/nuevo/']);
  }

  OnEditar(id: number){
    this._Route.navigate(['/dashboard/temporada/edit/', id.toString()]);
  }

  OnEliminar(id: number, estado : number){
    if(estado == 0){
      if (confirm("Esta seguro de eliminar esta Temporada?"))
      {
