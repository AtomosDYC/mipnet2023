...
export class TipopermisoListComponent implements OnInit {
  ...
  public loadData(): Observable<Response[] | null> {
    ...
    const datafull =  this.store.pipe(select(fromList.getTipoPermisos));

    if(this.filtro ){    
      const datafullfilter = datafull.pipe(
        map((data : Response[] | null) => data!.filter(valor => valor.wkf05nombre.toLowerCase().includes(this.filtro.toLowerCase())))
        )
  ...
  OnNuevo(){
    this._Route.navigate(['/dashboard/tipopermiso/nuevo/']);
  }
  ...
  OnEliminar(id: number, estado : number){
    if(estado == 0){
      if (confirm("Esta seguro de eliminar este tipo de Permiso?"))
      {
