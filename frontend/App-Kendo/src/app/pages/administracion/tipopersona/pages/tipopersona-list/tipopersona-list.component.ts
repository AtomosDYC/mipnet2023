...
export class TipopersonaListComponent implements OnInit {
  ...
  public loadData(): Observable<Response[] | null> {
    ...
    const datafull =  this.store.pipe(select(fromList.getTipoPersonas));

    if(this.filtro ){    
      const datafullfilter = datafull.pipe(
        map((data : Response[] | null) => data!.filter(valor => valor.per03nombre.toLowerCase().includes(this.filtro.toLowerCase())))
        )
  ...
  OnNuevo(){
    this._Route.navigate(['/dashboard/tipopersona/nuevo/']);
  }
