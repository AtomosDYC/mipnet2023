...
export class TipoparametroListComponent implements OnInit {
  ...
  public loadData(): Observable<Response[] | null> {
    ...
    const datafull =  this.store.pipe(select(fromList.getTipoParametros));

    if(this.filtro ){    
      const datafullfilter = datafull.pipe(
        map((data : Response[] | null) => data!.filter(valor => valor.wkf10nombre.toLowerCase().includes(this.filtro.toLowerCase())))
        )
  ...
  OnNuevo(){
    this._Route.navigate(['/dashboard/tipoparametro/nuevo/']);
  }
