...
import { ClienteEstacionRequest, ClienteEstacionResponse, ClienteEstacionRequestUpdate, clienteestaciondesactivateRequest, MenuEstacionResponse, menuclienteestacionRequest } from './save.models';
...
import { MenuEstacion } from '../../../../models/backend/clienteestacion/index';
...
export class SaveEffects {
  ...
  /*
  GetClienteEstacionmenu: Observable<Action> = createEffect(() =>
    this.actions.pipe(
      ofType(fromActions.Types.GET_CLIENTEESTACION_MENU),
      map((action: fromActions.GetClienteEstacionmenu) => action.menuclienteestacion),
      switchMap((request: menuclienteestacionRequest) =>
        this.httpClient.post<MenuEstacionResponse[]>(`${environment.url}api/clienteestacion/getmenuestaciones`, request)
          .pipe(
            map((menuestacion: MenuEstacionResponse[]) => new fromActions.GetClienteEstacionmenuSuccess(menuestacion)),
            catchError(err => {
              this.store.dispatch(fromvisibleToast.onError(err.error.errores.mensaje));
              return of(new fromActions.GetClienteEstacionmenuError(err.message));
            })
          )
      )
    )
