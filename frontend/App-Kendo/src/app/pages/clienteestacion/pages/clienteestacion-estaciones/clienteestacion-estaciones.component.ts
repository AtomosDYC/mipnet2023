...
import { EstacionResponse as Response, EstacionRequest as Request, EstacionbyidRequest as requestsearch } from './store/save';
...
import { SetSelect } from '../../../../models/backend/getselect/index';
...
@Component({
  selector: 'app-clienteestacion-estaciones',
  templateUrl: './clienteestacion-estaciones.component.html',
  styleUrls: ['./clienteestacion-estaciones.component.scss']
})
export class ClienteestacionEstacionesComponent implements OnInit {
...
*/
  ...
  public OnloadGrid(): void{
    ...
    this.store.dispatch(new fromList.ReadEstacion(this.requeststate));

    this.loading$ = this.store.pipe(select(fromList.getLoadingEstacion));

    this.data$ =  this.store.pipe(select(fromList.getEstaciones));
...
/*
    ...
    */
    this._Route.navigate(['/dashboard/clienteestacion/estaciones', this.ID ,'datosgenerales',this.ID]);
  }
      ...
      if(this.ID){
        ...
        const { cbxtipocomunicacion, cbxregion, cbxcomuna, txtdireccion, txtcasilla, txtcodigopostal, txtemail, txttelefono1,
          ...
          const UpdateRequest : Request = {
           ...
           this.store.dispatch(new fromList.CreateEstacion(UpdateRequest));
  ...
  OnEditarcomunicacion(id_cliente: number, id_comunicacion: number){
    ...
    if(this.ID){
      ...
      this.loading$ = this.store.pipe(select(fromList.getLoadingComunicacion));
        ...
        this.store.pipe(select(fromList.getEstacionbyidselector))
        .subscribe(data => {
          if(data){
  ...
  OnEliminarcomunicacion(id_cliente: number, id_comunicacion: number){
    ...
    if(this.ID){
      ...
      this.loading$ = this.store.pipe(select(fromList.getLoadingComunicacion));
        ...
        this.store.dispatch(new fromList.DeleteEstacion(DeleteRequest));
