...
{
    public class MonitorAccesoRepository : IMonitorAccesoRepository
    {
        ...
        public MonitorAccesoRepository(
        ...
        public async Task<PersonaAccesoResponseDto> GetMonitorAccesoById(int id)
        {
            using (var db = _contexto)
            {
                PersonaAccesoResponseDto? query = await (
                                   from mon in db.Mnt01Monitores
        ...
        public async Task<int> RegistroMonitorAcceso(PersonaAccesoRequestDto request)
        {
            ...
            if (resultado.Succeeded)
            {
                ...
                string sql = "EXEC pa_mipnet_monitoracceso_i @MNT01_LLAVE, @USERNAME, @PASSWORD, @USUARIO_ID ";

                List<SqlParameter> parms = new List<SqlParameter>
                {
                    // Create parameter(s)    
                    new SqlParameter { ParameterName = "@MNT01_LLAVE", Value = request.per01llave },
